...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for step, data in enumerate(data_loader):
        ...
        with amp_autocast():
            pred, node_features = model(f_in=data.x, pos=data.pos, batch=data.batch, 
                node_atom=data.z)
            pred = pred.squeeze()

            target_ori = (data.y[:, target] - task_mean) / task_std

            loss = criterion(pred, target_ori)
            for _ in range(num_repeat):
                ...
                mask_features = None
                target_value = (target_ori.detach(), z)
                loss_xyz = model(f_in=new_data.x, pos=new_data.pos, batch=new_data.batch, node_atom=new_data.z,
                    edge_d_index=new_data.edge_d_index, edge_d_attr=new_data.edge_d_attr, mask_node_features=mask_features, target=target_value, mask_atom=z, mask_position=xyz, node_head=True)
                loss += loss_xyz
...
def evaluate(model, norm_factor, target, data_loader, device, amp_autocast=None, 
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            with amp_autocast():
                pred, _ = model(f_in=data.x, pos=data.pos, batch=data.batch, 
                    node_atom=data.z,
