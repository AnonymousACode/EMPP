...
import my_datasets.md17 as md17_dataset
...
def get_args_parser():
    ...
    parser.add_argument('--xyz-weight', type=float, default=0.5)
    # random
...
class kl_circ(nn.Module):
    def __init__(self, res=100):
        super().__init__()
        # self.s2 = o3.ToS2Grid(1, (res, res + 1))
    ...
    def forward(self, x, label, method):
        if method == 'KL':
            return self.loss_KLD(x, label)
        elif method == 'L1':
            return self.L1(x, label)
        else:
            raise ValueError("FUCK loss")
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_err, train_loss = train_one_epoch(args=args, model=model, criterion=criterion,
            ...
            print_freq=args.print_freq, logger=_log, kl=kl)
...
def train_one_epoch(args, 
    ...
    for step, data in enumerate(data_loader):
        data = data.to(device)
        pred_y, pred_dy, node_features = model(node_atom=data.z, pos=data.pos, batch=data.batch)
        ...
        for i in range(len(data.ptr) - 1):
            # mask_num = int(0.2 * (data.ptr[i + 1] - data.ptr[i]))
            mask_num = 1
            # mask_tmp = torch.randperm(data.ptr[i + 1] - data.ptr[i])[:mask_num]
            # mask = torch.cat([mask, mask_tmp + data.ptr[i].cpu()])
            mask = torch.cat([mask, torch.randint(data.ptr[i], data.ptr[i + 1], (mask_num, ))])
            
        mask_inverse = torch.tensor([i for i in range(data.pos.size(0)) if i not in mask], device=data.pos.device)
        
        z = data.z[mask].detach()
        xyz = data.pos[mask].detach()
        energy_target = ((data.y - task_mean) / task_std).detach()
        force_target = (data.dy / task_std)[mask].detach()

        new_data = data.clone()
        new_data.pos = torch.index_select(data.pos, 0, mask_inverse)
        new_data.z = torch.index_select(data.z, 0, mask_inverse)
        new_data.batch = torch.index_select(data.batch, 0, mask_inverse)
        mask_features = None
        target_value = (energy_target, force_target, z)
        loss_xyz = model(pos=new_data.pos, batch=new_data.batch, node_atom=new_data.z, mask_node_features=mask_features, target=target_value, mask_atom=z, mask_position=xyz, node_head=True)
        loss += loss_xyz * args.xyz_weight
...
def evaluate(args, 
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(data_loader):
            ...
            data = data.to(device)
            pred_y, pred_dy, _ = model(node_atom=data.z, pos=data.pos, batch=data.batch, train_flag=False)
