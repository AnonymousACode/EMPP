...
class TransBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # self.e_embed = LinearRS(o3.Irreps('1x0e'), o3.Irreps('64x0e'))
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_scalars, 
        ...
        if target is not None:
            e, mask_atom = target
            # mask_atom = mask_atom.view(-1, self.mask_num)
            e_embed = self.e_embed(e)
            mask_atom = mask_atom.new_tensor([-1, 0, -1, -1, -1, -1, 1, 2, 3, 4])[mask_atom]
            mask_atom_embed, _, _ = self.atom_embed(mask_atom)
            mask_atom_embed = mask_atom_embed.view(e.shape[0], 2, -1)
            mask_atom_embed = mask_atom_embed.sum(dim=1)
            e_mask_embed = self.act_embed(e_embed + mask_atom_embed)
...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    def forward(self, pos, batch, node_atom, mask_node_features=None, target=None, mask_atom=None, mask_position=None, node_head=False, **kwargs) -> torch.Tensor:
        ...
        if not node_head:
            ...
            kl_loss = self.postion_prediction(pos, node_features, mask_node_features, mask_atom, mask_position, batch, target, edge_mask_index)
            return kl_loss
